...
export function DashboardPage(): JSX.Element {
  ...
  if (error) {
    return <EmptyState icon={ShieldAlert} title="NÃ£o foi possÃ­vel carregar a visÃ£o geral" description={error} />;
  }

  if (!data) {
    return <EmptyState icon={Activity} title="Carregando mÃ©tricas" description="Buscando a atividade mais recente do seu workspace." />;
  }
  ...
  return (
    <div className="space-y-7">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-accent">VisÃ£o geral</p>
          <h1 className="mt-2 text-2xl font-semibold text-ink md:text-3xl">Atividade do workspace</h1>
          <p className="mt-2 text-sm text-slate-600">Acompanhe o uso do agente, a seguranÃ§a das consultas e o retorno recebido.</p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Tempo mÃ©dio" value={formatDuration(overview.average_execution_time_ms)} icon={Clock3} tone="amber" />
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">DistribuiÃ§Ã£o das solicitaÃ§Ãµes registradas.</p>
          </div>
          <div className="mt-6 h-64">
            {statusData.length ? (
              ...
              <EmptyState icon={Activity} title="Sem consultas ainda" description="As consultas feitas pelo agente aparecerÃ£o aqui." />
            )}
          </div>
        </article>
        ...
        <article className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          ...
          <div className="mt-6 h-64">
            {tableData.length ? (
              ...
              <EmptyState icon={Activity} title="Sem tabelas consultadas" description="Tabelas aprovadas pelo validador aparecerÃ£o aqui." />
            )}
          </div>
        </article>
      </section>
